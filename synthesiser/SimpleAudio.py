...
class Audio(pyaudio.PyAudio):
    ...
    def time_stretch_fft(self, factor, windowsize=1024, overlap=512, apply_hanning=True):
        ...
        result = np.zeros( len(self.data) / factor + windowsize)

        for i in np.arange(0, len(self.data)-(windowsize+overlap), overlap*factor):
            ...
            i2 = int(i/factor)
            result[i2 : i2 + windowsize] += amp_window*a2_rephased

        result = ((2**(16-4)) * result/result.max())

        self.data = result.astype(self.nptype)

    def plot_waveform(self, array, x_unit="samples"):
    ...
    def plot_spectrum(self, array, plot_log=False):
        len_arr = len(array)
        #print len_arr
...
if __name__ == "__main__":
    ...
    #a.rescale(1.0)
    #a.add_echo(10,16000)
    #a.create_tone(440.0,480000,0.8)
    a.create_noise(128000,1)
    #a.play()
    #b.reverse_npindex()
    #b.plot_waveform(b.data, x_unit="time")
    #b.change_speed(2)
    #b.time_stretch_fft(0.8)
    a.play()
